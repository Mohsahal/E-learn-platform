...
function VideoPlayer({
  ...
  function handleProgress(state) {
    if (!seeking) {
      ...
      // Update max played time for restricted seeking
      if (!isAlreadyViewed && state.playedSeconds > maxPlayed) {
        setMaxPlayed(state.playedSeconds);
        // Inform parent about progress for server-side persistence
        const progressPercent = (state.playedSeconds / duration) * 100;
        if (progressPercent > 0 && isFinite(progressPercent)) {
  ...
  function handleForward() {
    if (isAlreadyViewed) {
      ...
      // For restricted: only allow forwarding up to maxPlayed
      const currentTime = playerRef?.current?.getCurrentTime();
      if (currentTime && isFinite(currentTime)) {
        const nextTime = Math.min(currentTime + 5, maxPlayed);
  ...
  function handleSeekMouseUp() {
    setSeeking(false);
    if (isFinite(played)) {
      const targetTime = played * duration;
      
      // RESTRICTION: Snapshot back if they try to skip ahead
      if (!isAlreadyViewed && targetTime > maxPlayed + 1) { // 1s buffer
        const snappedBackPlayed = maxPlayed / duration;
        setPlayed(snappedBackPlayed);
        playerRef.current?.seekTo(snappedBackPlayed);
  ...
  useEffect(() => {
    setPlayed(0);
    setCurrentTime(0);
    setMaxPlayed(0); // Reset max played for new lecture
