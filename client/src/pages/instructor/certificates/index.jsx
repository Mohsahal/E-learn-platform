    ...
    try {
      ...
      if (approvals?.success) {
        (approvals.data || []).forEach((a) => {
          if (a.approvedAt) map[a.studentId] = true;
          if (a.requestedAt) rMap[a.studentId] = true;
                        ...
                        students.map((s, i) => (
                          <tr key={`${s.studentId}-${i}`} className="hover:bg-white/[0.02] transition-colors group">
                            ...
                            <td className="px-5 py-4">
                              ...
                              ) : requestedMap[s.studentId] ? (
                                <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-amber-500/10 border border-amber-500/20 text-amber-400 text-xs font-medium animate-pulse">
                              ...
                              <Button 
                                ...
                                className={`h-8 px-4 rounded-lg text-xs font-medium transition-all ${
                                  approvedMap[s.studentId] 
                                    ...
                                    : requestedMap[s.studentId]
                                      ? 'bg-amber-600 hover:bg-amber-500 text-white shadow-md shadow-amber-500/20 border-none'
