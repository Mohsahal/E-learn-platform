...
function StudentViewCourseProgressPage() {
  ...
  const fetchCurrentCourseProgress = useCallback(async () => {
    ...
    if (response?.success) {
      if (!response?.data?.isPurchased) {
        ...
        const sequentialAccess = true; // FORCED: Sequential access is now mandatory for all courses.
  ...
  const handleProgressUpdate = useCallback(async (percentage) => {
    if (currentLecture && auth?.user?._id) {
      // Opt-in: only update if it's a significant change to reduce API calls
      // but for "perfect" unskippable we might wanted it more frequent.
      // Every 10% is usually fine.
       await updateVideoProgressService(
        auth?.user?._id,
        studentCurrentCourseProgress?.courseDetails?._id,
        currentLecture._id,
        percentage
  ...
  }, [auth?.user?._id, currentLecture, studentCurrentCourseProgress?.courseDetails?._id]);
  ...
  // Auto-redirect to certificate page after 5 seconds when course is completed
  useEffect(() => {
    let timeoutId;
    if (showCourseCompleteDialog && isCourseCompleted) {
      timeoutId = setTimeout(() => {
        navigate(`/learn/${id}?tab=certificate`);
      }, 5000);
    }
    return () => {
      if (timeoutId) clearTimeout(timeoutId);
    };
  }, [showCourseCompleteDialog, isCourseCompleted, navigate, id]);
        ...
        >
          <div className="aspect-video w-full bg-black/40 border-b border-white/5 shadow-2xl relative">
            <VideoPlayer
              ...
              isAlreadyViewed={studentCurrentCourseProgress?.progress?.some(p => p.lectureId === currentLecture?._id && p.viewed)}
            ...
            <div className="space-y-3">
              {studentCurrentCourseProgress?.courseDetails?.curriculum?.map(
                (lecture, index) => {
                  ...
                  const sequentialAccess = true; // FORCED: Sequential access is now mandatory.
            ...
            <div className="p-5 sm:p-10 space-y-6 overflow-y-auto pb-[max(1.25rem,env(safe-area-inset-bottom))]">
              ...
              <div className="space-y-3">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                   <Button
                      onClick={() => navigate(`/learn/${id}?tab=certificate`)}
                      className="bg-blue-600 hover:bg-blue-500 text-white font-black uppercase tracking-widest text-[10px] h-12 shadow-lg shadow-blue-600/20"
                   >
                     Claim Certificate
                   </Button>
                   <Button
