...
/**
 * Generate a unique student ID in the format: NXL-STU-XXXX
 * NXL = Nexora Learn (organization name)
 * STU = Student
 * XXXX = Unique sequential number starting from 1001
 */
async function generateUniqueStudentId() {
  const prefix = "NXL-STU-";
  ...
  if (lastStudent && lastStudent.studentId) {
    ...
    const match = lastStudent.studentId.match(/NXL-STU-(\d+)/);
    // Backward compatibility for old AST-STU format
    const oldMatch = lastStudent.studentId.match(/AST-STU-(\d+)/);
