...
const googleLogin = async (req, res) => {
  ...
  try {
    ...
    if (user) {
      ...
      let baseUserName = (name || email.split("@")[0]).trim().split(" ")[0];
      let uniqueUserName = baseUserName.substring(0, 25);
      
      const isUserNameTaken = await User.findOne({ userName: uniqueUserName });
      if (isUserNameTaken) {
        uniqueUserName = `${baseUserName.substring(0, 20)} ${Math.floor(1000 + Math.random() * 9000)}`;
      }
